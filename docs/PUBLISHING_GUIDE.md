...
2. Set `tag` to `1.0.0-rc.1`
3. Set `skip_all_in_one=true`
4. Keep `skip_docker=true`

When `1.0.0-rc.1` does not exist as a tag, the workflow stamps that version into the artifacts but checks out the triggering commit. This produces unsigned test artifacts without creating a release tag or publishing a production release.
...
Use `npm run release:windows-rc -- --version 1.0.0-rc.1 --skip-smoke` only after `npm run release:smoke` has already passed on the same checkout.
