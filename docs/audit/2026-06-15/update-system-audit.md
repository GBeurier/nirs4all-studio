...
>
> **P2-CP3 now implemented** (`scripts/smoke-self-update.cjs` + `electron/smoke-self-update.test.ts`, wired **non-blocking** into `release-unified.yml` for Linux + macOS x64/arm64). The three design problems were resolved, not deferred: (a) `is_online()` is not a network probe — just don't set `NIRS4ALL_OFFLINE` and point `NIRS4ALL_UPDATE_API_BASE` at a localhost fixture; (b) no second bundle — the N+1 asset is built from the installed bundle plus a sentinel (a minimal overlay tar.gz on Linux/Windows, a full ditto-zipped `.app` on macOS for bundle-mode replace); (c) rather than depend on the updater's relaunch carrying the forced port/HOME (it doesn't, via macOS `open -n`), the smoke verifies the result directly: sentinel present (replace happened) + it re-boots the updated bundle on the forced port (proves "the app works after the update", and on macOS proves the symlink-preserving `.app` extraction). The Electron run is CI-only; the pure parts (fixture server, asset build, update driver) have vitest coverage.
>
> **P3 (partial, 2026-06-16):** macOS `ditto -x -k --rsrc` extraction (signature/xattr-preserving, with a clean fallback) and `_find_installer_asset` (gated builds now download the right native `.exe`/`.dmg`/`.deb`/`.AppImage` via `installer_download_url`) are **done + green**. **Remaining:** wire the smoke for **Windows** + flip it to blocking after a `workflow_dispatch` validation run; AppImageUpdate / in-place AppImage refresh; and a dedicated "update needs admin" Windows message.
...
7. ✅ `smoke-self-update.cjs` — real-Electron self-update smoke, wired non-blocking into `release-unified.yml` (Linux + macOS x64/arm64); pure logic vitest-covered. ⏳ Windows wiring + flip to blocking after a `workflow_dispatch` validation run.

### P3 — harden the privileged & signed paths (partial)
8. Windows: real elevation UX — ✅ on declined UAC the updater now **relaunches the old app** (CP2) and the post-update reconciliation banner (CP3) surfaces the failure. ⏳ a dedicated "update needs admin" message is still nice-to-have.
9. ✅ macOS: `_extract_zip` prefers **`ditto -x -k --rsrc`** (symlink/xattr/signature-preserving, the inverse of the build's `ditto -c -k --sequesterRsrc`) with a fallback to the symlink-aware Python path; it wipes partial output before the fallback. Gatekeeper validation of the relaunched app is exercised by the macOS self-update smoke in CI.
10. ✅ Route gated builds to the **right native installer**: `_find_installer_asset` resolves the platform's `.exe`/`.dmg`/`.deb`/`.AppImage` and `download-info` exposes `installer_download_url`, so "Get installer" downloads it directly instead of only opening the release page. ⏳ AppImageUpdate / in-place AppImage refresh is still a larger follow-up.
