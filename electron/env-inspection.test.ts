...
describe("pinned recovery runtime readiness", () => {
  ...
  it("requires the recovery version when an older or newer nirs4all is already installed", () => {
    for (const version of ["0.9.3", "0.11.1", "1.1.3rc1"]) {
      expect(getUnsatisfiedExactPins(new Map([["nirs4all", version]]))).toEqual(["nirs4all==1.1.3"]);
    }
    expect(getUnsatisfiedExactPins(new Map([["nirs4all", "1.1.3"]]))).toEqual([]);
  });
});

// Real metadata resolution: no fake package imports or simulated JSON responses.
describe("Python distribution precedence", () => {
  it.each(["1.1.3", "1.0.1"])("keeps the first sys.path distribution (%s), including normalized aliases", async (firstVersion) => {
    ...
    try {
      ...
      for (const [directory, version, alias] of [
        ...
        [second, firstVersion === "1.1.3" ? "1.0.1" : "1.1.3", "example_package"],
      ]) {
      ...
      expect(getUnsatisfiedExactPins(inspected!.installedPackages)).toEqual(firstVersion === "1.1.3" ? [] : ["nirs4all==1.1.3"]);
    } finally {
      vi.unstubAllEnvs();
