...
export async function runBaselines(): Promise<BaselineReport> {
  ...
  logger.log('[PerformanceBaselines] Starting baseline measurements...');

  // 1. Initial render (500 samples)
  logger.log('[PerformanceBaselines] Testing initial render (500 samples)...');
  await whenIdle();
  ...
  logger.log('[PerformanceBaselines] Testing initial render (5000 samples)...');
  await whenIdle();
  ...
  logger.log('[PerformanceBaselines] Testing selection response...');
  await whenIdle();
  ...
  // 4. Frame rate
  logger.log('[PerformanceBaselines] Measuring frame rate...');
  await whenIdle();
  ...
  logger.log('[PerformanceBaselines] Baseline report:', report);
  logger.log(`[PerformanceBaselines] Score: ${score}% (${passed} passed, ${warnings} warnings, ${failed} failed)`);
