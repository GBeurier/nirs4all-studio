...
vi.mock('../visualizations', () => ({
  ...
  FoldDistributionChart: ({ y }: { y: number[] }) => <div data-testid="folds">{y.join(',')}</div>,
}));
...
const baseInput: FoldDistributionChartDataInput = {
  folds: null,
  y: [1, 2, 3],
};
...
describe('MainCanvasFoldDistributionPanel', () => {
  it('renders nothing when render state disables the folds chart', async () => {
  ...
  it('renders fold distribution content inside the chart panel and forwards panel actions', async () => {
    const onMaximize = vi.fn();
    const onHide = vi.fn();
    const { container, root } = await renderPanel({ stale: true, onMaximize, onHide });

    const panel = container.querySelector('section');
    expect(panel?.dataset.chartType).toBe('folds');
    expect(panel?.dataset.sampleCount).toBe('3');
    expect(container.querySelector('[data-testid="folds"]')?.textContent).toBe('1,2,3');
  ...
  it('renders the folds skeleton from render state', async () => {
    const { container, root } = await renderPanel({
      renderState: { ...baseRenderState, showSkeleton: true },
    });

    expect(container.querySelector('[data-testid="skeleton"]')?.textContent).toBe('folds');
