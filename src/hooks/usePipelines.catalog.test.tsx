...
describe("pipeline catalog shared with Run", () => {
  it("shows an imported pipeline immediately even when Run cached an empty catalog for five minutes", async () => {
    ...
    transport.get.mockImplementation((path: string) => {
      if (path === "/runs/pipelines") return Promise.resolve({ pipelines: [] });
      return Promise.resolve({ pipelines: [pipeline] });
    });
    ...
    try {
      ...
      expect(transport.get).toHaveBeenCalledWith("/runs/pipelines");
