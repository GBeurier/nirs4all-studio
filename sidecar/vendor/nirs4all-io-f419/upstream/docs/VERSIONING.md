...
| `AssembledDataset` summary/full wire | `ASSEMBLED_DATASET_VERSION` (core/Python) / `assembled_schema_version` | `2` | Bump when the JSON-compatible assembly artifact changes. v1 was unversioned and omitted source ids, scientific identity, and stable fold provenance. v2 readers must require integer `2`; re-bless all `*.assembled.canonical` fixtures in the same change. |
...
3. **Crate semver ≠ ABI version.** The cargo workspace version (`0.1.7`)
   tracks the publishable core/facade/C-ABI/CLI crates and synchronized language
   packages. The unpublished `nirs4all-io-dagml` bridge keeps its own version.
   The C ABI contract itself tracks `N4IO_ABI_VERSION`; it moves independently.
