...
JSON summaries and full exports of `AssembledDataset` carry
`assembled_schema_version: 2`, ordered feature `source_ids`, declared
sample/observation/repetition/group identity, and fold membership translated to
stable observation ids before partition reordering. The former unversioned wire
did not guarantee these fields and must not be interpreted as v2.
