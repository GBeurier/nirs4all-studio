...
/// Fold membership translated to stable observation ids before partitioning.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
...
impl AssembledDataset {
    ...
    /// `assembled_schema_version` distinguishes this public wire from DatasetSpec.
...
fn build_source_table(
    ...
    // Identity is never a feature/target role. Retain the aligned values as
    // metadata so downstream consumers never recreate identity from row order.
