...
use nirs4all_io_core::materialize::{
    assemble_in_memory, parse_fold_str, InMemorySource, SourcePayload,
};
...
impl ConfinedReader {
    fn new(dataset_root: &Path) -> Result<Self, SpecError> {
        ...
        F: FnOnce(),
    {
        ...
        if metadata.len() > MAX_ROLE_TAGGED_FILE_BYTES {
            return Err(role_err(format!(
                "{field} {raw:?} exceeds the {MAX_ROLE_TAGGED_FILE_BYTES}-byte file budget"
            )));
        }
        let next_total = self.total_bytes.saturating_add(metadata.len());
        if next_total > MAX_ROLE_TAGGED_TOTAL_BYTES {
            return Err(role_err(format!(
                "inputs exceed the {MAX_ROLE_TAGGED_TOTAL_BYTES}-byte aggregate budget"
            )));
        }
        ...
        file.by_ref()
            .take(MAX_ROLE_TAGGED_FILE_BYTES + 1)
            .read_to_end(&mut bytes)
            .map_err(|error| role_err(format!("cannot read {field} {raw:?}: {error}")))?;
        if bytes.len() as u64 > MAX_ROLE_TAGGED_FILE_BYTES {
            return Err(role_err(format!(
                "{field} {raw:?} grew beyond the {MAX_ROLE_TAGGED_FILE_BYTES}-byte file budget while reading"
            )));
        }
        let actual_total = self.total_bytes.saturating_add(bytes.len() as u64);
        if actual_total > MAX_ROLE_TAGGED_TOTAL_BYTES {
            return Err(role_err(format!(
                "inputs exceed the {MAX_ROLE_TAGGED_TOTAL_BYTES}-byte aggregate budget"
            )));
        }
...
pub fn load_role_tagged_assembled(
    ...
    let mut reader = ConfinedReader::new(dataset_root)?;
    ...
    assemble_in_memory(&spec, &sources, &HashMap::new(), fold_inline.as_deref())
}
...
mod role_tagged_secure_open_tests {
    ...
    fn directory_entry_swap_after_open_cannot_change_the_bytes_read() {
        ...
        let mut reader = ConfinedReader::new(root.path()).unwrap();
        let bytes = reader
