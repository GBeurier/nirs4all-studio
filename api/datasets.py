...
def _set_non_spectral_file_overrides(
    ...
    """Keep Y/metadata parsing independent from spectral-header inference.

    Spectral wavelength rows frequently look numeric and are correctly detected
    as ``has_header=False``. Reusing that result for target or metadata files
    turns their named headers into data. The dataset contract already supports
    per-file overrides, so persist the independently detected differences.
    """
    warnings: list[str] = []
